    ...
    <table>
      ...
      <tbody>
        ...
        <template v-else v-for="req in requests" :key="req.id">
          <tr v-if="req.recordType === 'request' || req.applications.length > 0" class="approval-req">
            <td>{{ req.date }}</td>
            <td>{{ req.username }}</td>
            <td>
              <div v-for="appDetail in req.applications" :key="req.id + '-' + appDetail.typeKey">
                {{ appDetail.typeName }}
              </div>
              </td>
            <td>
              <div v-for="appDetail in req.applications" :key="req.id + '-' + appDetail.typeKey + '-status'">
                {{ getStatusText(appDetail.status) }}
              </div>
              </td>
            <td>
              <template v-for="appDetail in req.applications" :key="req.id + '-' + appDetail.typeKey + '-action'">
                <div v-if="appDetail.status === 0">
                  <button @click="confirmAction(req, appDetail.typeKey, 'approve')">
                    {{ appDetail.typeName }}承認
                  </button>
                  <button @click="confirmAction(req, appDetail.typeKey, 'reject')">
                    {{ appDetail.typeName }}却下
                  </button>
                </div>
              </template>
              </td>
...
const selectedTypeKey = ref(''); // 承認/却下する申請タイプキー (e.g., 'paid', 'early')
...
const confirmAction = (req, typeKey, action) => {
  selectedRequest.value = req;
  selectedTypeKey.value = typeKey;
  selectedAction.value = action;
  
  // DTOからtypeNameを取得
  const typeDetail = req.applications.find(app => app.typeKey === typeKey);
  const typeName = typeDetail ? typeDetail.typeName : '不明な';
  
  dialogMessage.value = `${typeName}申請を${action === 'approve' ? '承認' : '却下'}しますか？`;
  showConfirmDialog.value = true;
};

// 確認ダイアログで「はい」が押された時の処理（承認/却下）
const executeAction = async () => {
  if (!selectedRequest.value || !selectedTypeKey.value || !selectedAction.value) return;

  // CombinedApplicationDtoのrequestIdを使用
  ...
  try {
    if (action === 'approve') {
      url = `${API_BASE_URL}/admin/request/${requestId}/approve/${typeKey}`;
      successMessage = '申請が承認されました。';
      errorMessage = '申請の承認中にエラーが発生しました。';
    } else { // action === 'reject'
      url = `${API_BASE_URL}/admin/request/${requestId}/reject/${typeKey}`;
